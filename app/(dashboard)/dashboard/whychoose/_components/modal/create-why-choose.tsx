...
import { createWhyChoose } from "@/actions/createWhyChoose";
import { useModalWhyChoose } from "@/hooks/use-modal-store-why-choose";

export const CreateWhyChooseModal = () => {
  const form = useForm({
    resolver: zodResolver(createWhyChooseSchema),
    defaultValues: {
      title: "",
      description: "",
      img: "",
  ...
  const { isOpen, onClose, type } = useModalWhyChoose();

  const isModalOpen = isOpen && type === "createWhyChoose";

  const isLoading = form.formState.isSubmitting;

  const onSubmit = async (values: z.infer<typeof createWhyChooseSchema>) => {
    try {
      const res = await createWhyChoose(values);
