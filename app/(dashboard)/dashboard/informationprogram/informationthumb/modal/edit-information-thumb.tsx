...
import { editInformationThumb } from "@/actions/editInformationProgram";

export const EditInformationThumbModal = () => {
  ...
  const isModalOpen = isOpen && type === "editInformationThumb";
  ...
  const onSubmit = async (values: z.infer<typeof editProcessSchema>) => {
    try {
      //   api
      const res = await editInformationThumb(values);
      console.log("res", res);
