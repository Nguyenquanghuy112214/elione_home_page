...
import { createInformationThumb } from "@/actions/createInformationProgram";

export const CreateInformationThumbModal = () => {
  const form = useForm({
  ...
  const isModalOpen = isOpen && type === "createInformationThumb";

  const isLoading = form.formState.isSubmitting;

  const onSubmit = async (values: z.infer<typeof createProcessSchema>) => {
    try {
      const res = await createInformationThumb(values);
