...
import { deleteInformationProgram } from "@/actions/deleteInformationThumb";

export const DeleteInformationProgramModal = () => {
  const { isOpen, onClose, type, data } = useModalProcess();

  const isModalOpen = isOpen && type === "deleteInformationProgram";
  ...
  const onClick = async () => {
    try {
      setIsLoading(true);
      const res = await deleteInformationProgram({ id });
      if (!!res.success) {
