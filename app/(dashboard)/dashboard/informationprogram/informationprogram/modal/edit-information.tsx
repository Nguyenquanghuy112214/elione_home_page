...
import { editInformationProgram } from "@/actions/editInformationProgram";

export const EditInformationProgramModal = () => {
  ...
  const isModalOpen = isOpen && type === "editInformationProgram";
  ...
  const onSubmit = async (values: z.infer<typeof editProcessSchema>) => {
    try {
      //   api
      const res = await editInformationProgram(values);
      console.log("res", res);
