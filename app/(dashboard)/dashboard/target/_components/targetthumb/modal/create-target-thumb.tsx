...
import { createTargetThumb } from "@/actions/createTarget";

export const CreateTargetThumbModal = () => {
  const form = useForm({
  ...
  const isModalOpen = isOpen && type === "createTargetThumb";

  const isLoading = form.formState.isSubmitting;

  const onSubmit = async (values: z.infer<typeof createProcessSchema>) => {
    try {
      const res = await createTargetThumb(values);
