...
import { deleteTargetThumb } from "@/actions/deleteTarget";

export const DeleteTargetThumbModal = () => {
  const { isOpen, onClose, type, data } = useModalProcess();

  const isModalOpen = isOpen && type === "deleteTargetThumb";
  ...
  const onClick = async () => {
    try {
      setIsLoading(true);
      const res = await deleteTargetThumb({ id });
      if (!!res.success) {
