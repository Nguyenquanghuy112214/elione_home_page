...
import { deleteEvaluate } from "@/actions/deleteEvaluate";
import { useModalEvaluate } from "@/hooks/use-modal-evaluate";

export const DeleteEvaluateModal = () => {
  const { isOpen, onClose, type, data } = useModalEvaluate();

  const isModalOpen = isOpen && type === "deleteEvaluate";
  const { id, title } = data;
  ...
  const onClick = async () => {
    try {
      setIsLoading(true);
      const res = await deleteEvaluate({ id });
      if (!!res.success) {
