...
import { editExcellent } from "@/actions/editExcellent";

export const EditExcellentModal = () => {
  ...
  const isModalOpen = isOpen && type === "editExcellent";
  ...
  const onSubmit = async (values: z.infer<typeof editProcessSchema>) => {
    try {
      //   api
      const res = await editExcellent(values);
      console.log("res", res);
