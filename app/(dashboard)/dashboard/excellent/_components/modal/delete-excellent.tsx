...
import { deleteExcellent } from "@/actions/deleteExcellent";

export const DeleteExcellentModal = () => {
  const { isOpen, onClose, type, data } = useModalProcess();

  const isModalOpen = isOpen && type === "deleteExcellent";
  ...
  const onClick = async () => {
    try {
      setIsLoading(true);
      const res = await deleteExcellent({ id });
      if (!!res.success) {
