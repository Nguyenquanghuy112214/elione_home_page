...
export const EditStepModal = () => {
  const { isOpen, onClose, type, data } = useModalStep();
  const { title, description, img, id, subtitle } = data;
  ...
  }, [form, title, description, subtitle, img, id]);
  ...
  const onSubmit = async (values: z.infer<typeof editStepSchema>) => {
    try {
      //   api
      const res = await editStep(values);
      if (!!res.success) {
