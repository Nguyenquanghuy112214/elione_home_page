...
import { deleteReceive } from "@/actions/deleteReceive";
import { useModalReceive } from "@/hooks/use-modal-store-receive";

export const DeleteReceiveModal = () => {
  const { isOpen, onClose, type, data } = useModalReceive();

  const isModalOpen = isOpen && type === "deleteReceive";
  const { id, title } = data;
  ...
  const onClick = async () => {
    try {
      setIsLoading(true);
      const res = await deleteReceive({ id });
      if (!!res.success) {
