...
import { createSheduleThumb } from "@/actions/createSheduleThumb";

export const CreateScheduleThumbModal = () => {
  const form = useForm({
  ...
  const isModalOpen = isOpen && type === "createScheduleThumb";

  const isLoading = form.formState.isSubmitting;

  const onSubmit = async (values: z.infer<typeof createProcessSchema>) => {
    try {
      const res = await createSheduleThumb(values);
