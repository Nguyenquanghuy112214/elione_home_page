...
import { deleteScheduleThumb } from "@/actions/deleteScheduleThumb";

export const DeleteScheduleThumbModal = () => {
  const { isOpen, onClose, type, data } = useModalProcess();

  const isModalOpen = isOpen && type === "deleteScheduleThumb";
  const { id, title } = data;
  ...
  const onClick = async () => {
    try {
      setIsLoading(true);
      const res = await deleteScheduleThumb({ id });
      if (!!res.success) {
