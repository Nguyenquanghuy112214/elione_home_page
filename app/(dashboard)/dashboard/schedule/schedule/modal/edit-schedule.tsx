...
import { editSchedule } from "@/actions/editScheduleThumb";

export const EditScheduleModal = () => {
  const { isOpen, onClose, type, data } = useModalSchedule();
  const { title, id } = data;
  ...
  const isModalOpen = isOpen && type === "editSchedule";
  ...
  const onSubmit = async (values: z.infer<typeof editScheduleSchema>) => {
    try {
      //   api
      const res = await editSchedule(values);
      if (!!res.success) {
